...
"""Services base that PROVIDES the Keycloak bearer token; the caller attaches it.

The generated service wrappers (``ondewo/survey/client/services/*.py``) subclass :class:`ServicesInterface` and
expose a single ``stub`` property. They define no RPC methods, so nothing here attaches
:attr:`ServicesInterface.metadata` automatically -- **the caller must pass it explicitly on
every call**::
...
offline-token flow, :attr:`ServicesInterface.metadata` carries a freshly auto-refreshed
